    ...
    <!-- 本表的查询条件 -->
        ...
        function setDataRow(item, number) {
            ...
            row.appendChild(yuangongEmailCell);
                ...
                //员工合同
            var yuangongFileCell = document.createElement('td');
            if(item.yuangongFile ==null || item.yuangongFile == '' || item.yuangongFile == 'null'){
                    yuangongFileCell.innerHTML = '没有下载文件';
            }else {
                var yuangongFileFile = document.createElement('button');
                    yuangongFileFile.innerHTML = '下载';
                var yuangongFileFileAttr = "download('" + item.yuangongFile + "')"
                    yuangongFileFile.setAttribute('onclick',yuangongFileFileAttr);
                    yuangongFileCell.appendChild(yuangongFileFile);
            }
            row.appendChild(yuangongFileCell);


            //入职考核分数
            var yuangongFenshuCell = document.createElement('td');
            yuangongFenshuCell.innerHTML = item.yuangongFenshu;
            row.appendChild(yuangongFenshuCell);


                //员工备注
            var yuangongContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.yuangongContent && item.yuangongContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.yuangongContent.indexOf('<img');
                var endIndex = item.yuangongContent.indexOf('>');
                    yuangongContentCell.innerHTML = item.yuangongContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.yuangongContent != null && item.yuangongContent != "" && item.yuangongContent.length >= 11) {
                        yuangongContentCell.innerHTML = item.yuangongContent.substring(0, 10) + "...";
                } else {
                        yuangongContentCell.innerHTML = item.yuangongContent;
                }
            }
            row.appendChild(yuangongContentCell);
